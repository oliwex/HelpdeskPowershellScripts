...
$pathToTemplateFile="C:\test\hello.vz"
$pathToResultFile="C:\test\hello.png"

$dependencyGraph=graph g {
    @(Get-ADDirectReports -Identity hmyjak -Recurse | Select-Object -Unique SamAccountName,ManagerAccount).ForEach({edge $_.ManagerAccount -To $_.SamAccountName })
}

Set-Content -Path $pathToTemplateFile -Value $dependencyGraph

Export-PSGraph -Source $pathToTemplateFile -Destination $pathToResultFile